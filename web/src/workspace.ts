...
export function download(name: string, value: unknown, type = 'application/json'): void {
  ...
  dialog.innerHTML = `<div class="dialog-head"><h2 id="export-title">Export ready.</h2><button class="icon-button" aria-label="Close export">×</button></div><p class="export-filename"></p><p class="card-sub">Review the contents, then download the file or copy the full text. This export stays on the local computer.</p><label class="field">Report preview<textarea readonly spellcheck="false"></textarea><small></small></label><div class="export-actions"><a class="primary" download>Download file ↓</a><button class="secondary copy-export" type="button">Copy full text</button><button class="secondary select-export" type="button">Select full text</button></div><p class="status-line" role="status">No file has been saved yet.</p>`;
  ...
  link.onclick = () => { status.textContent = 'Download requested. Check the browser’s downloads; if it does not save, use Copy full text.'; };
...
function weather(): void {
  ...
  const decision=firstBeyond ? `The 95th percentile of along-track disagreement first exceeds the ${num(tolerance)} km tolerance at ${num(+firstBeyond[0],0)} hours. ${lastWithin?`The previous measured lead is ${num(+lastWithin[0],0)} hours.`:'It already exceeds it at the shortest measured lead.'} A fresher orbit is needed to support this tolerance beyond that sampled lead.` : 'No measured lead exceeds this tolerance. This does not establish performance between samples or on another spacecraft.';
  $('content').innerHTML=header('Space weather / model evaluation','Orbit confidence.','Compare a public orbit prediction with a reference, then check whether a storm correction reduces the disagreement.', '<button class="secondary" id="setup-inline">Run on this PC ↗</button><button class="primary" id="export-weather">Export evidence ↓</button>')+
    `<div class="toolbar"><div class="segmented" role="group" aria-label="Benchmark window">${names.map(k=>`<button data-window="${esc(k)}" aria-pressed="${k===selectedWindow}">${esc(imported?k:(titles[k]||k))}</button>`).join('')}</div><label class="input-inline">Along-track tolerance <input id="tolerance" type="number" min="0.001" max="100000" step="any" value="${tolerance}"/> km</label></div>`+
  ...
  $('setup-inline').textContent=localEngine?'Compare local files ↗':'Use local data ↗';
