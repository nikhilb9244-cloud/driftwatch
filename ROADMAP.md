...
- **Stage C should interpolate the SpaceX ephemeris states directly for served events, so the trajectory and the covariance share a source.** Today the geometry comes from CelesTrak's SGP4 fit to those ephemerides while the covariance comes from the ephemerides themselves, and the two disagree by a published RMS of about 0.2 km — larger than SpaceX's own sigma for the first several hours. That residual is currently carried in quadrature on every served covariance (`SPACEX_SGP4_FIT_RMS_KM`), which is the honest patch but not the fix: interpolating the states removes the fit from the chain entirely, at which point the residual term goes to zero and the miss distance itself improves. It needs the position blocks kept at fetch (only the covariance is stored now), an interpolator for a 60-second state grid, and a decision about what Stage B screens on.
