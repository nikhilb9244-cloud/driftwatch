...
| `primary_trajectory`, `secondary_trajectory` | string | Which trajectory produced that object's state at the time of closest approach: `sgp4` (its element set through SGP4) or `spacex-ephemeris` (the operator's published states, interpolated). Both stages use the same trajectory, so this also says what the pair was *screened* on. It is the record the risk step reads to decide whether the SGP4 fit residual belongs on the event's covariance — see `cov_source_secondary` below. |
| `refine_method` | string | `root` (range-rate root inside a sign-change bracket), `minimum` (golden-section fallback) or `scan` (a hundred-point sweep of one sample interval, used where the served trajectory jumps — at the start of an ephemeris's coverage, at its 48-hour seam and at its 72-hour horizon — because a discontinuous separation has neither a bracketed root nor a unimodal minimum). A `scan` row's time of closest approach is placed to about a hundredth of the Stage B step rather than to the root finder's microsecond tolerance. |
...
| `cov_source_primary`, `cov_source_secondary` | string | The model's source label for each, **per event** (a scenario wrapper prefixes its own, e.g. `scaled:4:default:leo`; a storm scenario appends `+storm:<label>`, and `model_version` carries `storm/<scenario>/2` for the rule under which operator-controlled objects are not displaced, `/1` for the rule before 2026-09-05). Three values matter for a Starlink secondary. `spacex-ephemeris`: their published covariance served *and* the event's geometry came from their published states, so trajectory and covariance share a source and nothing is added. `spacex-ephemeris+sgp4-fit`: their covariance served but the geometry still came from CelesTrak's SGP4 fit to their file, so the residual of that fit is added in quadrature — the two are then different trajectories and the distance between them is a real, independent error. The base model's own label (`default:leo` and so on) past the file's 72-hour horizon. Before Phase 4 Step 1 an object whose events straddled the horizon got one combined label, `spacex-ephemeris+default:leo`, on all of them; now each event carries what actually served it. `model_version` carries `spacex-ephemeris/3` for the per-event rule, `/2` for the older residual-on-everything rule and `/1` for as-published. |
