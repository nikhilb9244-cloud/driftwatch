...
Earth source imagery remains **4096 × 2048**. Antialiasing, vector charts/icons, higher device-pixel rendering and a detail selector improve sharpness. Auto uses the device ratio capped at 2×, High requests 2.5× and Low power uses 1×, all bounded to about eight million canvas pixels. The app reports actual rendered dimensions. Higher rendering resolution cannot recover surface detail absent from the image; this is not an 8K texture claim. Local analysis loads no external images or fonts.
