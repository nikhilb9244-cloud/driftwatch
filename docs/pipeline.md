...
| **`risk` for a scenario, x4** | **16.9 min each, 67 min** | CPU: two density tracks per **object in an event**, from its own epoch to the window end | 88 to 122 min |
...
**The total is about 1 h 53 m locally and 2 h 20 m to 3 h 0 m on a runner, against the six-hour
job limit** -- 39 to 50 % of it, with three to three and two-thirds hours spare. `timeout-minutes`
...
each, per scenario. Measured twice: 1,012 s over 2,944 objects on the 2026-09-03 run and 990 s
over 2,993 objects on the 2026-09-01 one, so **0.34 s per object per scenario**.
...
| At 0.34 s per object per scenario, four scenarios, x1.3 to x1.8 | **7,300 to 10,400 objects in events** |
