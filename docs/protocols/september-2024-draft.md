# September 2024 energy-error and propagation experiment

**Status: UNFROZEN DRAFT — not a registration, execution authorisation, or completed experiment.** No freeze time, source-hash manifest or result is asserted by this document. September reference-residual and post-burn outcome access remains pending the conditions below. A separate dated JSON protocol and matching frozen Markdown record the final scientific choices; author attestation remains an independent prerequisite for data access.
